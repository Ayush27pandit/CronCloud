...
export const JobModel = {
  ...
  async create(jobData) {
    const db = await dbPromise;
    const { name, url, method, headers, body, cronExpression, secret } =
      jobData;
    const id = uuidv4();

    await db.run(
      `INSERT INTO jobs (id, name, url, method, headers, body, cronExpression, secret) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        name,
  ...
  async update(id, jobData) {
    const db = await dbPromise;
    const { name, url, method, headers, body, cronExpression, status, secret } =
      jobData;
